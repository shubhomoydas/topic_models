...
my ($communications, $reference2idx) = load_communications($userCommunicationsFile);
...
for (my $i = 5; $i < 100; $i++) {
	...
	my $groupsFile = "$inGroupsPath/random-groups2users-$i.csv";
	my $groupCommunicationCountsFile = "$outPath/random-groups2users-$i-counts.csv";

	my $groups = load_groups($groupsFile, $reference2idx);
	my @groupids = keys %$groups;
	my $ngroups = @groupids;
	my @refids = keys %$reference2idx;
	my $nrefs = @refids;
	print ("$i: Loaded $ngroups groups, $nrefs references...\n");
...
sub load_communications($) {
	...
	return ($communications, $reference2idx);
